...
import com.qunar.hive.jdbc.dao.JdbcRecordIterator;
...
public class JdbcRecordReader implements RecordReader<LongWritable, MapWritable> {
  ...
  public JdbcRecordReader(JobConf conf, JdbcInputSplit split) throws IOException {
    ...
    try {
      ...
      iterator = dbAccessor.getRecordIterator(conf, split);
        ...
        if ((record != null) && (!record.isEmpty())) {
          for (Entry<String, Object> entry : record.entrySet()) {
            if (entry.getValue() == null) {
              value.put(new Text(entry.getKey()), NullWritable.get());
            } else {
              value.put(new Text(entry.getKey()), new ObjectWritable(entry.getValue()));
            }
          }
          return true;
        } else {
