...
import com.qunar.hive.jdbc.utils.Tuple2;
...
public class JdbcRecordWriter implements RecordWriter {
  ...
  public JdbcRecordWriter(JobConf conf) throws HiveJdbcDatabaseAccessException {
    ...
    switch (dbType) {
      case MYSQLQMHA:
        ((QmhaJdbcDatabaseAccessor) dbAccessor).setWrite(true);
        LOGGER.info("qmha database access set write = true");
        break;
      case MYSQLPXC:
        ((PxcJdbcDatabaseAccessor) dbAccessor).setWrite(true);
        LOGGER.info("pxc database access set write = true");
        break;
      default:
        break;
    }
